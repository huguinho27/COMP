	...
	public void saveFile(String name) throws IOException {
		...
		try {
			...
			for (int i = 0; i < nodes.size(); i++) {
				...
				if (elseTo != null) {
					if (to != null)
						out.write("\"" + i + ": " + nodes.get(i).getContent() + "\"" + " -> " + "\"" + to + ": "
								+ nodes.get(Integer.parseInt(to)).getContent() + "\"" + "[label=\"true\"]" + "\n");
					out.write("\"" + i + ": " + nodes.get(i).getContent() + "\"" + " -> " + "\"" + elseTo + ": "
							+ nodes.get(Integer.parseInt(elseTo)).getContent() + "\"" + "[label=\"false\"]" + "\n");
				} else {
					if (to != null)
						out.write("\"" + i + ": " + nodes.get(i).getContent() + "\"" + " -> " + "\"" + to + ": "
								+ nodes.get(Integer.parseInt(to)).getContent() + "\"" + "\n");
				}
			}
			out.write("}\n");
		} finally {
